    ...
    <NuxtLink :to="{ name: 'catalog-id', params: { id: complex?.id }, hash: '#main-content' }" :class="['wrapper cursor-pointer', $attrs.class]">
        <div :class="['building transition-all duration-300 md:rounded-[3px] h-full flex flex-col', shadow ? 'building-shadow' : '']">
            ...
            <div :class="['px-[10px] md:px-[21px] text-sm pt-3 md:pt-[18px] pb-4', shadow ? '' : 'md:border-l-[1.6px] md:border-r-[1.6px] md:border-[#1da95826]']">
                <div class="flex justify-between flex-wrap mb-[5px] md:mb-[8px] gap-2">
                    ...
                    <complex-stage :count-homes="complex.count_homes" class="hidden md:flex" />
                </div>
            ...
            <div class="mt-auto p-[21px] bg-[#F4FAFF] text-sm hidden md:block">
                <p class="mb-[22px] flex items-center justify-between flex-wrap text-[#3478F6] font-[Inter]">
                    <slot name="foot-ad">
                        <span class="text-[13px] font-semibold leading-[15px]">{{ complex?.count_free_apartments }} квартир</span>
                        <span class="text-[14px] font-normal leading-4">от {{ format_thousands(startingPrice(complex)) }} ₽</span>
                        <!-- <span class="text-[14px] font-normal leading-4">от 64 335 ₽/мес</span> -->
                    </slot>
                </p>

                <button :class="['block text-center py-4 font-[Raleway] rounded-[3px] border-[1.6px] text-[13px] leading-[15px] font-bold border-[#FCBD00] text-[#131313] w-full z-50 relative hover-bg', whiteCta ? 'bg-white' : 'bg-transparent ']">
