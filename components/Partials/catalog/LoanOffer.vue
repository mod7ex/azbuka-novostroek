...
<template>
    <div>
        <!-- <pre> {{ banks }} </pre> -->
        <Card v-for="({ name, logo }, i) in banks" :key="i" class="bg-[#F9F9F9] py-[14px] px-[18px] rounded-[5px] md:rounded-none mb-[10px] md:mb-5">
            <template #header>
                <div class="flex items-center justify-between mb-[13px]">
                    <!-- <app-img :src="logo" class="w-[67px] h-[24px] md:w-[91px] md:h-[43px]" /> -->
                    <p>{{ name }}</p>
